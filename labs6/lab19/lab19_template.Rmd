...
MaxAndMin <- function(f, x, y, s) {
  ...
  #   x: start of sequence
  #   y: end of sequence
  #   s: sequence increment
  # Return: 
  #   list of max of functionized vector and minimum of numbers that produce max
...
# <Your fix here>
ss <- function(x) {
  # Returns the cumulative max and median of a vector
  # 
  # Args: 
  #   x: vector
  # Return: 
  #   list of cumulative mean and median
  x <- x[!is.na(x)]
  lst <- x[1]
  for (i in 2:length(x)) {
    lst[i] <- lst[i - 1] + x[i]
  }
  cummean <- lst / (1:length(x))
  lst <- x
  cummedian <- 0
  for(i in 1:length(lst)) {
    ...
    c <- sort(x)
    if(length(x) %% 2 == 0) {
      cummedian[i] <- (c[floor((length(c) + 1) / 2)] + 
                         c[ceiling((length(c) + 1) / 2)]) / 2
    } else {
      cummedian[i] <- c[c(length(c) + 1) / 2]
    }
  }
  return(list(cummean, cummedian))
}
