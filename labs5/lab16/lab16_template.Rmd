...
```{r}
# <Your code>
df <- data.frame(gbp_rates, exrate_times)
ggplot(df, aes(exrate_times, gbp_rates)) +
  geom_line() +
  labs(x = "Date",
       y = "Exchange Rates (against Euro)",
...
```{r}
# <Your code>
GetExchangeRates <- function(abbrev) {
  exrate_root %>% 
    xml_find_all(str_c('//Cube[@currency="', abbrev, '"]')) %>% 
    xml_attr("rate") %>% 
    as.numeric() %>% 
    return()
}
...
# <Your code>
exrate.df %>% 
  gather(Country, Exchange_Rate, USD, CAD, AUD, GBP) %>% 
  ggplot(aes(time, Exchange_Rate, group = Country, color = Country)) +
  geom_line() +
  labs(x = "Date",
       y = "Exchange Rates (against Euro)",
       title = "Exchange Rates Over Time",
