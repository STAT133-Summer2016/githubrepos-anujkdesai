...
Sandwich2 <- function(n) { 
  return(str_detect(n, "[0-9]*([0-9])[0-9]\\1[0-9]*"))
}
...
# Your code here
BigMac <- function(n) {
  return(str_detect(n, "[0-9]*([0-9])[0-9]{2}\\1[0-9]*"))
}
```
...
MSandwich <- function(n, m) {
  str <- str_c("[0-9]*([0-9])[0-9]{", as.character(m), "}\\1[0-9]*")
  return(str_detect(n, str))
}
```
...
# Your code here
LuhnSum <- function(n){
  reverse_n <- as.numeric(unlist(str_split(as.character(n), "")))[nchar(n):1]
  odd <- 0
  even <- 0

  for (i in 1:length(reverse_n)) {
    if (i %% 2 == 1){
      odd <- sum(odd, reverse_n[i])
    } else if (reverse_n[i] >= 5) {
      even <- sum(even, (reverse_n[i]*2 - 9))
    } else {
      even <- sum(even, (reverse_n[i]*2))
    }
  }
...
```{r, warning = FALSE}
# Your code here
group_states <- split(states, nchar(states))
names(group_states) <- c("charcount4", "charcount5", "charcount6", "charcount7", 
                         "charcount8", "charcount9", "charcount10", "charcount11", 
                         "charcount12", "charcount13", "charcount14")
states_list <- lapply(group_states, function(i) {
  if (str_length(i) %% 2 == 1) {
    str_to_upper(i)
  } else {
    str_to_lower(i)
...
```{r, eval = T}
# Your code here
files <- list.files("/Users/anujdesai510/stat133/hw3/babynames")
year <- str_subset(files, "[:digit:]{4}")
fun <- function(n) {
  str_c("babynames/", n) %>% 
    read_csv(col_names = c("name", "sex", "n"),
           col_types = "cci") %>% 
    mutate(year = str_extract_all(n, "[:digit:]{4}"))
  
}
yob_df <- lapply(year, fun)
babynames <- do.call(rbind, yob_df)
babynames <- mutate(babynames, year = str_replace_all(babynames$year, "[.]?[a-z]", ""))
head(babynames)
```
...
top90 <- head(baby90, 6)
pop90 <- top90[with(top90, order(sex, name)), ]
```
...
```{r, echo = T}
# Your code here
top1000males <- babynames %>% 
  filter(year >= 2006 & year <= 2015 & sex == "M") %>% 
  arrange(-n)
top1000females <- babynames %>% 
  filter(year >= 2006 & year <= 2015 & sex == "F") %>% 
  arrange(-n)
```
